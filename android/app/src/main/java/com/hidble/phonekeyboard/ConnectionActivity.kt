...
class ConnectionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 按目标显隐：蓝牙版显示"手机模拟键盘"卡片；外接板版（Pico/ESP32）只显示板子卡片
        val isBt = BuildConfig.TARGET_MODE == "bt"
    ...
    private fun refreshBoardStatus(connected: Boolean) {
        ...
        if (connected) {
            ...
            boardStatusText.text = "已连接：${boardLabel()} Keyboard（“发送到键盘”将走板子输出）"
            ...
            boardStatusText.text = "未连接：点下面按钮扫描并连接 ${boardLabel()} Keyboard"
